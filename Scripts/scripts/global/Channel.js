...
var Channel = (function () {
    ...
    Channel.prototype.beforeMessage = function (src, msg) {
    };
    Channel.prototype.toString = function () {
        return "[object POChannel]";
    };
    Channel.prototype.setTopic = function (src, topicInfo) {
    ...
    Channel.prototype.isChannelOwner = function (id) {
    ...
    Channel.prototype.isChannelAdmin = function (id) {
        if(!sys.dbRegistered(sys.name(id))) {
            return false;
        }
    ...
    Channel.prototype.isChannelOperator = function (id) {
        if(!sys.dbRegistered(sys.name(id))) {
            return false;
        }
    ...
    Channel.prototype.addRole = function (src, tar, group, data) {
        var name = tar.toLowerCase();
        var auth = typeof src == "string" ? src : sys.name(src);
        if(sys.dbIp(tar) === undefined) {
            return [
                "self", 
                "The user '" + tar.toCorrectCase() + "' doesn't exist!"
            ];
        }
        if(!sys.dbRegistered(tar) && [
            ...
            return [
                "self", 
                "The user '" + tar.toCorrectCase() + "' is not registered so you can't give them channel authority!"
            ];
        }
        if(typeof this.operators != 'object') {
            this.operators = [];
        ...
        if(group == "owner") {
            if(this.masters.indexOf(name) > -1) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is already a channel owner!"
                ];
            }
            if(this.masters.length > 10) {
                return [
                    "self", 
                    "There is a limit of 10 owners!"
                ];
            }
            this.masters.push(name);
            return [
                "all", 
                sys.name(src) + " made " + tar.toCorrectCase() + " a channel owner!"
            ];
        }
        if(group == "admin") {
            if(this.admins.indexOf(name) > -1) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is already a channel admin!"
                ];
            }
            if(this.admins.length > 50) {
                return [
                    "self", 
                    "There is a limit of 50 admins!"
                ];
            }
            this.admins.push(name);
            return [
                "all", 
                sys.name(src) + " made " + tar.toCorrectCase() + " a channel admin!"
            ];
        }
        if(group == "mod") {
            if(this.operators.indexOf(name) > -1) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is already a channel mod!"
                ];
            }
            if(this.operators.length > 100) {
                return [
                    "self", 
                    "There is a limit of 100 mods!"
                ];
            }
            this.operators.push(name);
            return [
                "all", 
                sys.name(src) + " made " + tar.toCorrectCase() + " a channel mod!"
            ];
        }
        if(group == "member") {
            if(this.members.indexOf(name) > -1) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is already a member!"
                ];
            }
            if(this.members.length > 250) {
                return [
                    "self", 
                    "There is a limit of 250 members!"
                ];
            }
            this.members.push(name);
            return [
                "all", 
                sys.name(src) + " made " + tar.toCorrectCase() + " a member!"
            ];
        }
        if(group == "muted") {
            if(this.isPunished(name) !== "none") {
                return [
                    "self", 
                    tar.toCorrectCase() + " is already " + this.isPunished(name) + " from this channel!"
                ];
            }
            if(!this.hasPermission(src, tar)) {
                return [
                    "self", 
                    tar.toCorrectCase() + " has equal or higher auth than you, so you can't channel mute them!"
                ];
            }
            if(this.official && !this.isChannelOwner(src)) {
                ...
                if(data.reason === "") {
                    return [
                        "self", 
                        "You need to provide a reason for the channel mute!"
                    ];
                }
            }
            this.muted[name] = {
            ...
            return [
                "all", 
                auth + " muted " + tar.toCorrectCase() + timestring + " in this channel!" + (data.reason !== "" ? " [Reason: " + data.reason + "]" : "")
            ];
        }
        if(group == "banned") {
            if(this.isPunished(name) === "banned") {
                return [
                    "self", 
                    tar.toCorrectCase() + " is already banned from this channel!"
                ];
            }
            if(!this.hasPermission(src, tar)) {
                return [
                    "self", 
                    tar.toCorrectCase() + " has equal or higher auth than you, so you can't channel ban them!"
                ];
            }
            if(this.official && !this.isChannelOwner(src)) {
                ...
                if(data.reason === "") {
                    return [
                        "self", 
                        "You need to provide a reason for the channel ban!"
                    ];
                }
            }
            this.banned[name] = {
            ...
            return [
                "all", 
                auth + " banned " + tar.toCorrectCase() + timestring + " from this channel!" + (data.reason !== "" ? " [Reason: " + data.reason + "]" : "")
            ];
        }
        return [
            "self", 
    ...
    Channel.prototype.removeRole = function (src, tar, group) {
        var name = tar.toLowerCase();
        if(typeof this.operators != 'object') {
            this.operators = [];
        }
        ...
        if(group == "owner") {
            if(this.masters.indexOf(name) == -1) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is not a channel owner!"
                ];
            }
            ...
            return [
                "all", 
                sys.name(src) + " removed " + tar.toCorrectCase() + " from the channel owner list!"
            ];
        }
        if(group == "admin") {
            if(this.admins.indexOf(name) == -1) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is not a channel admin!"
                ];
            }
            ...
            return [
                "all", 
                sys.name(src) + " removed " + tar.toCorrectCase() + " from the channel admin list!"
            ];
        }
        if(group == "mod") {
            if(this.operators.indexOf(name) == -1) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is not a channel mod!"
                ];
            }
            ...
            return [
                "all", 
                sys.name(src) + " removed " + tar.toCorrectCase() + " from the channel mod list!"
            ];
        }
        if(group == "member") {
            if(this.members.indexOf(name) == -1) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is not a member!"
                ];
            }
            ...
            return [
                "all", 
                sys.name(src) + " removed " + tar.toCorrectCase() + " from the channel member list!"
            ];
        }
        if(group == "muted") {
            if(!this.muted.hasOwnProperty(name)) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is not muted in this channel!"
                ];
            }
            delete this.muted[name];
            return [
                "all", 
                sys.name(src) + " unmuted " + tar.toCorrectCase() + " in this channel!"
            ];
        }
        if(group == "banned") {
            if(!this.banned.hasOwnProperty(name)) {
                return [
                    "self", 
                    tar.toCorrectCase() + " is not banned from this channel!"
                ];
            }
            delete this.banned[name];
            return [
                "all", 
                sys.name(src) + " unbanned " + tar.toCorrectCase() + " from this channel!"
            ];
        }
        return [
            "self", 
    ...
    Channel.prototype.issueAuth = function (src, name, group) {
    ...
    Channel.prototype.takeAuth = function (src, name, group) {
    ...
    Channel.prototype.register = function (name) {
    ...
    Channel.prototype.canJoin = function (id) {
        if(this.isBanned(id)) {
            return "banned";
        }
    ...
    Channel.prototype.canTalk = function (id) {
        return !this.isMuted(id);
    };
    Channel.prototype.ban = function (src, tar, data) {
    ...
    Channel.prototype.unban = function (src, tar) {
    ...
    Channel.prototype.mute = function (src, tar, data) {
    ...
    Channel.prototype.unmute = function (src, tar) {
    ...
    Channel.prototype.isMuted = function (id) {
        if(this.isChannelOperator(id)) {
            return false;
        }
    ...
    Channel.prototype.isPunished = function (name) {
        var banlist = this.banned;
    ...
    Channel.prototype.hasPermission = function (src, tar) {
        var srcauth = 0;
        if(typeof src == "string") {
            ...
            if(this.isChannelOwner(src)) {
                ...
                if(this.isChannelAdmin(src)) {
                    srcauth = 2;
                } else {
                    if(this.isChannelOperator(src)) {
                        srcauth = 1;
                    }
                }
            }
    ...
    Channel.prototype.chanAuth = function (name) {
        var maxauth = 0;
        ...
        if(this.masters.indexOf(lname) > -1) {
            ...
            if(this.admins.indexOf(lname) > -1 && maxauth < 2) {
                maxauth = 2;
            } else {
                if(this.operators.indexOf(lname) > -1 && maxauth < 1) {
                    maxauth = 1;
                }
            }
        }
    ...
    Channel.prototype.changeParameter = function (src, parameter, value) {
        if(parameter == "topic") {
            this.topic = value;
    ...
    Channel.prototype.getReadableList = function (type) {
        try  {
            var name = "";
            var mh = {
            };
            ...
            for(var x in mh) {
                if(mh.hasOwnProperty(x)) {
                    ...
                    tmp.push([
                        ...
                        auth, 
                        issuetime, 
                        isNaN(mh[x].expiry) ? expirytime : Utilities.getTimeString(parseInt(expirytime)), 
                        reason
                    ]);
                }
            }
