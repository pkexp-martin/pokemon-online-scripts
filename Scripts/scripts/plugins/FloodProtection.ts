...
export class FloodProtection implements IPlugin {

    public sourcePath: string;

    public beforeNewPM(src: number): string {
    ...
    public afterChatMessage(src: number, message: string, channel: number): string {
        var user = SESSION.users(src);
        ...
        if (!poChannel.ignorecaps && this.isCapsed(message) && userMayGetPunished) {
            ...
            if (user.caps >= maxCaps && !user.mute.active) {
                /* Should never happen with typescript
                ...
                */
                ...
                if (officialChan) {
                    ...
                    poChannel.mute("CapsBot", sys.name(src), { 'time': time, 'reason': "Overusing CAPS" });
                }
            }
        } else if (user.caps > 0) {
            user.caps -= 1;
        ...
        if (!poChannel.ignoreflood && userMayGetPunished) {
            ...
            if (user.floodcount > linecount) {
                ...
                if (officialChan) {
                    ...
                    poChannel.mute("KickBot", sys.name(src), { 'time': 3600, 'reason': "Flooding" });
...
export var instance: IPlugin = new FloodProtection();