...
export class TierCheck implements IPlugin {

    public sourcePath: string;

    private static bannedGSCSleep = [
        sys.moveNum("Spore"),
    ...
    public beforeChangeTier(src, team, oldtier, newtier) {
    ...
    public handleCommandDwReleased(channel: number, src: number, commandData: string, tar: number): string {
    ...
    public addCheck(exclusive, tiers, checker) {
        this.checkers.push({ tiers: tiers, checker: checker, exclusive: exclusive });
    };

    private isLegal(src, team, tier, silent = false) {
...
// TODO: Get rid of all harcoded lists
...
export var instance = new TierCheck();

instance.addCheck(
    EXCLUDING,
    challenge_cups,
    function eventMovesCheck(src, team) {
        var ret = [];
        for (var i = 0; i < 6; i++) {
...
instance.addCheck(
    INCLUDING,
    ["Wifi LC", "Wifi LC Ubers", "Wifi UU LC"],
...
instance.addCheck(
    EXCLUDING,
    ["No Preview OU", "No Preview Ubers"],
...
instance.addCheck(
    INCLUDING,
    ["No Preview OU", "Wifi OU", "Wifi UU", "Wifi LU", "Wifi LC", "DW LC", "Wifi Ubers", "No Preview Ubers", "Clear Skies", "Clear Skies DW", "Monotype", "Monocolour", "Monogen", "Smogon OU", "Smogon UU", "Smogon RU", "Wifi NU", "Metronome"],
...
instance.addCheck(
    INCLUDING,
    ["Smogon OU", "Wifi OU", "No Preview OU"],
...
instance.addCheck(
    INCLUDING,
    ["Wifi UU", "Wifi LU", "Wifi NU", "Wifi LC"],
...
instance.addCheck(
    EXCLUDING,
    [],
    function eventShinies(player, team) {
        var beasts = {};
...
instance.addCheck(
    EXCLUDING,
    challenge_cups,
    function hasOneUsablePokemon(player, team) {
        for (var slot = 0; slot < 6; slot++)
